...
__attribute__((__target__("thumb"))) void payload_start(int dummy0, int dummy1, int dummy2, const uint8_t *address)
{
    (void)dummy0;
    (void)dummy1;
    (void)dummy2;
...
__attribute__((__noinline__, __target__("arm"))) static uint32_t DisableInterrupts()
{
    uint32_t oldCPSR;
    __asm__(
        "mrs %0, CPSR\n\t"
...
__attribute__((__noinline__, __target__("arm"))) static void RestoreInterrupts(uint32_t oldCPSR)
{
    __asm__ volatile(
        "msr CPSR_c, %0\n\t"
        :
